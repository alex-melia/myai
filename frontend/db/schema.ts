...
export const users = pgTable("users", {
  ...
  tokens: integer("tokens").default(5000),
...
export const accounts = pgTable(
  ...
  (table) => ({
    fkUser: foreignKey({
      ...
      .onDelete("cascade")
      .onUpdate("cascade"),
    userIndex: uniqueIndex("user_id_idx").on(table.user_id),
  })
)
...
export const interface_settings = pgTable(
  "interface_settings",
  {
    id: uuid("id").primaryKey().defaultRandom(),
...
export const inquiries = pgTable(
  ...
  (table) => ({
    ...
    fkInquiringUser: foreignKey({
      columns: [table.inquiring_user_id],
      foreignColumns: [users.id],
    }),
  })
)
