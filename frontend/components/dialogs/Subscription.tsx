...
export default async function Subscription() {
  ...
  const subscriptionStatus = currentUser?.subscription_status?.toString()

  return (
    <Dialog>
      <DialogTrigger asChild>
        <div className="flex items-center text-blue-500 justify-center mt-8 text-center hover:bg-gray-100 p-2 w-fit rounded-md cursor-pointer">
          <Zap className="mr-2 size-4" />
          <p className="font-semibold text-sm">Join Premium to Buy Tokens!</p>
        </div>
      </DialogTrigger>
        ...
        <div className="flex flex-col lg:flex-row gap-12 w-full sm:p-8">
          ...
          <Card className="flex flex-col items-center w-full shadow-md">
            ...
            <CardFooter>
              ...
              >
                <DialogClose asChild>
                  <Button disabled={subscriptionStatus === "ACTIVE"}>
                    Upgrade
                  </Button>
                </DialogClose>
              </Link>
